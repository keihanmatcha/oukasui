このエラーは、コードの中に\*\*「全角スペース」や「Webサイト特有の空白文字（ノーブレークスペース）」\*\*が紛れ込んでいるために発生しています。見た目は普通の空白に見えますが、Pythonはこれをプログラムの一部として認識できずにエラーになります。

特に、コピー＆ペーストした箇所のインデント（行頭の空白）が原因です。

**以下のコードを「すべてコピー」して、`update_archives.py`の中身をこれに完全に置き換えてください。**
（不要な文字をすべて標準のスペースに置換・修正した「完全版」です）
