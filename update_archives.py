提示されたコードには、**Pythonが停止してしまう文法エラー（SyntaxError）が1箇所あります。また、コピー＆ペーストの際に混入しやすい不正な空白文字**が含まれている可能性があります。

以下の修正を行いました。

1.  **コンマ漏れの修正**: `TAG_CONVERSION_MAP` の「にじさんじマリカ杯」の行末に `,`（コンマ）が抜けていました。これが原因で動かなくなっています。
2.  **空白文字の清掃**: 全角スペースや特殊な空白文字をすべて削除し、正しいインデントに整形しました。

このコードを**すべてコピーして貼り直せば**確実に動作します。
